-- =========================================================================
-- 0027 - a dispensa de um alerta automatico vale ate o fim do dia
--
-- PROBLEMA: dispensar um alerta era definitivo. O `on conflict do update` do
-- refresh_alerts atualizava titulo e severidade mas nunca tocava em
-- dismissed_at, entao uma OS atrasada ficava em silencio para sempre depois
-- que alguem fechou o aviso uma vez. A propria docs/13-ALERTAS.md dizia o
-- contrario ("alertas dispensados voltam se a condicao continuar valendo").
--
-- REGRA NOVA: a dispensa vale pelo dia corrente (fuso America/Sao_Paulo).
-- No dia seguinte, se a condicao ainda existir, o alerta reaparece.
--
-- Por que nao "volta no proximo recalculo": o recalculo roda a cada abertura
-- do dashboard, entao dispensar duraria segundos — o botao perderia sentido.
-- Por que nao "so quando piorar": uma OS que fica atrasada por semanas nunca
-- mais avisaria.
-- =========================================================================
...
begin
  ...
  on conflict (alert_key) do update
    set severity = excluded.severity,
        ...
        updated_at = now(),
        -- Dispensa de ontem (ou antes) caduca: o problema continua de pe.
        dismissed_at = case
          when alerts.dismissed_at is not null
           and (alerts.dismissed_at at time zone 'America/Sao_Paulo')::date < v_today
          then null
...
comment on function public.refresh_alerts is
  'Recalcula os alertas automaticos. Dispensa vale ate o fim do dia: se a condicao '
  'continuar valendo no dia seguinte, o alerta reaparece.';
